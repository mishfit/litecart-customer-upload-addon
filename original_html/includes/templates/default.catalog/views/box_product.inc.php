<article id="box-product" class="box" data-id="<?php echo $product_id; ?>" data-sku="<?php echo htmlspecialchars($sku); ?>" data-name="<?php echo htmlspecialchars($name); ?>" data-price="<?php echo currency::format_raw($campaign_price ? $campaign_price : $regular_price); ?>">
  ...
  <div class="row">
    ...
    <div class="col-sm-8 col-md-6">
      ...
      <?php } ?>
<!-- added code -->
<span class="lek-code"><?php echo language::translate(__CLASS__.'show_code', 'Code')?>: #<?php echo $product_id; ?></span>
<!-- end added code -->
      ...
      <div class="buy_now" style="margin: 1em 0;">
        <?php echo functions::form_draw_form_begin('buy_now_form', 'post', false, true); ?>
        <?php echo functions::form_draw_hidden_field('product_id', $product_id); ?>

        <?php $is_custom_image_set =0; if ($options) {  ?>
          <?php foreach ($options as $option) { if($option['name']=="Custom Print Image"){$is_custom_image_set =1;} ?>
          <div class="form-group <?php echo 'class_'.str_replace(" ","",$option['name']); ?>">
        ...
        <?php
          $category_id = reference::product($product_id)->default_category_id;
          $parent_cat_name = reference::category($category_id)->parent_id;
          if($is_custom_image_set){
        ?>
        <div class="custom-image form-group" style="">
                  <label><?php echo language::translate('print_image', 'Upload Image for Print'); ?></label>
                  <input type="file" class="form-control" name="print_image[]" id="Print_image" multiple="multiple">
        </div>
        <script>
          $("#Print_image").on("change", function() {
                if ($("#Print_image")[0].files.length > 4) {
